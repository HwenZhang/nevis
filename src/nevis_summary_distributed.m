...
f1 = figure(1); clf;
ax(1) = subplot(6,1,1);
    plot(t,sum(Q_b_in),'b-',t,sum(Q_b_out),'r-',LineWidth=1.5);
    hold on;
...
ax(2) = subplot(6,1,2); 
    if isfield(tt,'pts_phi') && ~isempty([tt.pts_phi])  
    plot(t,mean(pts_N),'-',LineWidth=1.5);
    end
    hold on
...
ax(4) = subplot(6,1,4);
    yyaxis left
    plot(t,mean(hs_b),'b-',LineWidth=1.5);    
    ...
    yyaxis right
    plot(t,mean(Sx_b),'r-',LineWidth=1.5);
...
ax(5) = subplot(6,1,5); 
    yyaxis left
    plot(t,sum(V_b),'b-',LineWidth=1.5);
    xlabel('t [ d ]');
    ylabel('V [ m^3 ]');
...
ax(6) = subplot(6,1,6); 
    p1 = plot(t,mean(p_b),'k-',LineWidth=1.5); % blister pressure + \rho_i g h
    hold on
    % p2 = plot(t,pts_N,'b-.',LineWidth=1.5);
    p3 = plot(t,mean(p_w),'r-.',LineWidth=1.5); 
