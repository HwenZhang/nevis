...
% out = nevis_channel_timeseries_plot(casename, 'TimeRange', [0 20])
%
...
% Drainage-relative time is absolute model time minus TimeZero. The default
% TimeZero is 750 d. TimeRange is drainage-relative time in days.
...
all_saved_frames = local_saved_frames(result_dir);
frames = opts.Frames;
if isempty(frames)
    frames = all_saved_frames;
end
if isempty(frames)
...
for k = 1:n_frames
    ...
    t_abs(k) = local_time_days(vv.t, ps, pd);
    t(k) = t_abs(k) - t_zero;
...
for i = 1:2:numel(varargin)
    ...
    switch name
        ...
        case 'timezero'
            opts.TimeZero = local_validate_time_zero(value);
        case 'pointnode'
...
function time_zero = local_validate_time_zero(value)
if ~isnumeric(value) || ~isscalar(value) || ~isfinite(value)
    error('nevis_channel_timeseries_plot:BadTimeZero', ...
        'TimeZero must be one finite scalar in absolute model days.');
end
time_zero = value;
end
