function [tt, vv, info, state] = nevis_run_regional(casename)
    %NEVIS_RUN_REGIONAL Run the standardized NEVIS regional workflow.
    %
    ...
    if nargin < 1 || isempty(casename)
        error('nevis_run_regional:MissingCaseName', ...
            'Call nevis_run_regional with a casename.');
    end
    ...
    % Read the case config. Looks under cases/<casename>.m or
    % cases/<casename>/<casename>.m (per-case subdir layout).
    ...
    if ~strcmp(cfg.casename, casename)
        error('nevis_run_regional:CaseNameMismatch', ...
