...
casename = 'n2d_regional_racmo_eps1e_02_kappa1e_10_mu2e1_partition8e_01_k01e_01_spinup';  % specify the case name
...
Q_b_in = sum(pd.Q_0*[tt.Qb_in],1);               % dimensional influx (m^3/s)
...
xlabel('t [ d ]');
ylabel('Q [ m^3/s ]');
h=legend('Q_{b,in}','Q_{b,relax}','Q_{outb}','\Delta Q_{outQ}','\Delta Q_{outq}','Q_{in}','NumColumns',2,'Units','normalized');
h.Position = [0.75, 0.92, 0.2, 0.05];
h.Color = 'none';
...
ax = nexttile(leftLayout);
yyaxis left
    plot(ax,t,mean(hs_b,1),'b-',LineWidth=1.5);    
...
yyaxis right
    plot(ax,t,0.25*(mean(Sx_b+Sy_b+Ss_b+Sr_b,1)),'r-',LineWidth=1.5);
    hold on
...
yyaxis left
    h1 = plot(ax,t,h_b(end,:),'b-',LineWidth=1.5, DisplayName='h_b');
    hold on
    h2 = plot(ax,t,h_s(end,:),'b--',LineWidth=1.5, DisplayName='h_s');
    % plot(ax,t,hb_analytical,'b--',LineWidth=1.5);
    xlabel('t [ d ]');
    ylabel('h [ m ]');
    text(0.025,0.8,'(e) h_b+h_s and p_b at GPS','Units','normalized','FontSize',14)

yyaxis right
    plot(ax,t,p_b(end,:)/1e6,'r-',LineWidth=1.5); % pressure at the blister
    hold on
    ...
    grid on
    grid minor
    legend([h1,h2],{'h_b','h_s'},'Location','northeast');

% panel (f)
ax = nexttile(leftLayout); 
yyaxis left
    plot(ax,t,R_b/1e3,'b-',LineWidth=1.5); % blister pressure + \rho_i g h
    hold on
...
% Rb_analytical = (3/pi*V_b./h_b).^(1/2);
    ...
    hold on
    % plot every lake drainage time
    % for i = 1:length(pp.t_drainage)
    %     td = pp.t_drainage(i);
    %     ts = pp.t_duration(i);
    %     if pp.V_l(i)*(ps.Q0*ps.t) < 5e8
    %         continue
    %     end
    %     xline((td-1.5*ts)*ps.t/pd.td,'--k','LineWidth',1.0); % dashed line
    % end
    % ylim([0 1.1e8])
    ylabel('V_b [ m ]');
    h = legend('R_b','V_b','NumColumns',2,'Location','southeast');
    h.Color = 'none';
...
hold on
plot(ax,xx(oo.pts_ni(end)),yy(oo.pts_ni(end)),'ko', 'MarkerSize',6,'MarkerFaceColor',"k", 'MarkerEdgeColor', 'k'); % mark lake location
...
log_size = max(log10(markersize_l + eps),eps);
hscatter = scatter(ax, xl, yl, 200*log_size, 'ko', 'filled', 'MarkerFaceColor',"b", 'MarkerEdgeColor', 'k'); % mark lake location
...
%% make video
v = VideoWriter(['./results/videos/' casename '_slow'],'MPEG-4');
...
for i_idx = 1:length(frame_indices)
    ...
    vv2 = nevis_nodedischarge(vv2,aa,pp,gg,oo); % calculate node discharge
    qnet = ps.qs*(vv2.qs + vv2.qe + vv2.qQ + 0*vv2.Q);
    disp(['The blister outflux is ' num2str(vv2.Q_outb*ps.Q0) ' m^3/s.']);
    ...
    markersize_l = lake_input(time,pp,ps);
    log_size = max(log10(markersize_l + eps),eps);
    hscatter.SizeData = 400*log_size;
