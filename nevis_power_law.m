...
growth_phase = t>200.0; % Define the growth phase for the power law fit
...
Rb_a = (8.42*pd.B*1000*Q0^3/pd.mu)^(1/10)*((t0-200.0)*pd.td).^(0.4);

figure("Position",[100,100,800,600]);
subplot(3,1,1)
plot(t0-200.0, R_b0, 'k-', 'LineWidth', 2);
hold on
xlabel('Time (days)');
ylabel('Blister radius (m)');
title('Blister radius vs time');
grid on;
set(gca, 'FontSize', 14);
% Fit a power law to the data
log10t = log10(t0-200.0);
log10R_b = log10(R_b0);
p = polyfit(log10t, log10R_b, 1);
% Calculate the fitted values
fitted_Rb = 10.^(polyval(p, log10(t0-200.0)));
% Plot the fitted line
plot(t0-200.0, fitted_Rb, 'r--', 'LineWidth', 2);
plot(t0-200.0, Rb_a, 'b--', 'LineWidth', 2);
% yyaxis right
%         plot(t0-200.0, V2d, 'g-', 'LineWidth', 2);
%         hold on
%         plot(t0-200.0, Q0*((t0-200.0)*pd.td), 'm--', 'LineWidth', 2);
...
xlabel('Time (days)');
ylabel('Blister radius (m)');
title('Blister radius vs time(loglog)');
grid on;
set(gca, 'FontSize', 14);
% Fit a power law to the data
log10t = log10(t0-200.0);
...
fitted_Rb = 10.^(polyval(p, log10(t0-200.0)));
% Plot the fitted line
loglog(t0-200.0, fitted_Rb, 'r--', 'LineWidth', 2);
...
subplot(3,1,3)
loglog(t0-200.0, hb_max0, 'k-', 'LineWidth', 2);
hold on
xlabel('Time (days)');
ylabel('Maximum blister thickness (m)');
title('Maximum blister thickness vs time(loglog)');
grid on;
set(gca, 'FontSize', 14);
% Fit a power law to the data
log10hb_max = log10(hb_max0);
p = polyfit(log10t, log10hb_max, 1);
% Calculate the fitted values
fitted_hb_max = 10.^(polyval(p, log10(t0-200.0)));
% Plot the fitted line
loglog(t0-200.0, fitted_hb_max, 'r--', 'LineWidth', 2);
legend('Maximum blister thickness', 'Fitted power law', 'Location', 'NorthWest');
% Display the power law exponent
exponent_hb = p(1);
text(0.02, 0.5, sprintf('Power law exponent: %.2f', exponent_hb), 'Units', 'normalized', 'FontSize', 14, 'Color', 'r');
% Save the figure
saveas(gcf, [oo.rn '/blister_power_law.png']);