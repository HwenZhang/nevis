...
subplot(3,1,1)
plot(t0-inject_time, R_b0, 'k-', 'LineWidth', 2);
hold on
xlabel('Time (days)');
...
log10t = log10(t0-inject_time);
log10R_b = log10(R_b0);
p = polyfit(log10t, log10R_b, 1);
% Calculate the fitted values
fitted_Rb = 10.^(polyval(p, log10(t0-inject_time)));
% Plot the fitted line
plot(t0-inject_time, fitted_Rb, 'r--', 'LineWidth', 2);
% plot(t0-inject_time, Rb_a, 'b--', 'LineWidth', 2);
% yyaxis right
...
fitted_Rb = 10.^(polyval(p, log10(t0-inject_time)));
% Plot the fitted line
loglog(t0-inject_time, fitted_Rb, 'r--', 'LineWidth', 2);
...
subplot(3,1,3)
loglog(t0-inject_time, hb_max0, 'k-', 'LineWidth', 2);
hold on
xlabel('Time (days)');
...
% Calculate the fitted values
fitted_hb_max = 10.^(polyval(p, log10(t0-inject_time)));
% Plot the fitted line
loglog(t0-inject_time, fitted_hb_max, 'r--', 'LineWidth', 2);
