...
oo.casename = 'n1d_test_DX31_25'; % casename
...
%% supraglacial lakes
pp.x_l = [0.1*L/ps.x];                                          % x-coord of lakes
pp.y_l = [0];                                                   % y-coord of lakes
pp.V_l = [1e7/(ps.Q0*ps.t)];                                    % volume of lakes         
pp.t_drainage = [0.25*pd.td/ps.t];                              % time of lake drainages (assumed to be the middle time of the Gaussian)
...
oo.t_span = (2e-4:2e-4:1.0)*pd.td/ps.t;          % time span for simulation (in ps.t)
