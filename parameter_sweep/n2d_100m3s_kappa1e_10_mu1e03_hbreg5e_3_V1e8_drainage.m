...
casename = 'n2d_100m3s_kappa1e_10_mu1e03_hbreg5e_3_V1e8_drainage'; % drainage system filename
...
oo.include_blister = 1;                      % blister uses its own active domain in the current implementation
oo.include_ice = 0;                          % do not solve ice-sheet dynamics in this reference run
...
gg.n1_blister = gg.n1m;                           % label all blister edge nodes as boundary nodes
...
pp.x_l = [0.5*L/ps.x];                                          
pp.y_l = [0.5*W/ps.x];                                          
pp.V_l = [1e8/(ps.Q0*ps.t)];                           % different V_l values
