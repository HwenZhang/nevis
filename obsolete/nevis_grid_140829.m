...
if isfield(oo,'method') && strcmp(oo.method,'r_stress_l_vel_tb_ref'), oo.yreflective = 1; else oo.yreflective = 0; end
...
for i = 1:nI
    ...
    if oo.yreflective, 
        for j = 1, nconnect(i+(j-1)*nI,:) = [ i+(j-1)*eI i+1+(j-1)*eI i+(j)*fI i+j*fI ]; end
        for j = nJ, nconnect(i+(j-1)*nI,:) = [ i+(j-1)*eI i+1+(j-1)*eI i+(j-1)*fI i+(j-1)*fI ]; end
    end
end
...
for i = 1:eI
    ...
    if oo.yreflective
    for j = 1,
        if i==1,            econnect(i+(j-1)*eI,:) = [ nI+(j-1)*nI i+(j-1)*nI i+(j)*cI i+j*cI ];
        elseif i==eI,       econnect(i+(j-1)*eI,:) = [ i-1+(j-1)*nI 1+(j-1)*nI i+(j)*cI i+j*cI ];
        else                econnect(i+(j-1)*eI,:) = [ i-1+(j-1)*nI i+(j-1)*nI i+(j)*cI i+j*cI ]; 
        end
    end
    for j = eJ,
        if i==1,            econnect(i+(j-1)*eI,:) = [ nI+(j-1)*nI i+(j-1)*nI i+(j-1)*cI i+(j-1)*cI ];
        elseif i==eI,       econnect(i+(j-1)*eI,:) = [ i-1+(j-1)*nI 1+(j-1)*nI i+(j-1)*cI i+(j-1)*cI ];
        else                econnect(i+(j-1)*eI,:) = [ i-1+(j-1)*nI i+(j-1)*nI i+(j-1)*cI i+(j-1)*cI ]; 
        end
    end
    end
end
...
if oo.yreflective, 
    % for reflective bcs, s and r must be switched on boundary : these definitions assume nmeans and
    % nmeanr will always be used together
    nmeans = nmeans + sparse([ntopbdy; ntopbdy],[econnect(nconnect(ntopbdy,1),3); econnect(nconnect(ntopbdy,2),4)],[-1/2*ones(length(ntopbdy),1); 1/2*ones(length(ntopbdy),1)],nIJ,cIJ) ...
        + sparse([nbotbdy; nbotbdy],[econnect(nconnect(nbotbdy,1),3); econnect(nconnect(nbotbdy,2),4)],[1/2*ones(length(nbotbdy),1); -1/2*ones(length(nbotbdy),1)],nIJ,cIJ);
    nmeanr = nmeanr + sparse([ntopbdy; ntopbdy],[econnect(nconnect(ntopbdy,1),4); econnect(nconnect(ntopbdy,2),3)],[1/2*ones(length(ntopbdy),1); -1/2*ones(length(ntopbdy),1)],nIJ,cIJ) ...
        + sparse([nbotbdy; nbotbdy],[econnect(nconnect(nbotbdy,1),4); econnect(nconnect(nbotbdy,2),3)],[-1/2*ones(length(nbotbdy),1); 1/2*ones(length(nbotbdy),1)],nIJ,cIJ);
end
...
if oo.yreflective, 
    % for reflective bcs, s and r must be switched on boundary : these definitions assume ndds and
    % nddr will always be used together
    ndds = ndds + sparse([ntopbdy; ntopbdy],[econnect(nconnect(ntopbdy,1),3); econnect(nconnect(ntopbdy,2),4)],[Dx(ntopbdy).^(-1).*Dy(ntopbdy).^(-1); Dx(ntopbdy).^(-1).*Dy(ntopbdy).^(-1)],nIJ,cIJ) ...
        + sparse([nbotbdy; nbotbdy],[econnect(nconnect(nbotbdy,1),3); econnect(nconnect(nbotbdy,2),4)],[-1*Dx(nbotbdy).^(-1).*Dy(nbotbdy).^(-1); -1*Dx(nbotbdy).^(-1).*Dy(nbotbdy).^(-1)],nIJ,cIJ);
    nddr = nddr + sparse([ntopbdy; ntopbdy],[econnect(nconnect(ntopbdy,1),4); econnect(nconnect(ntopbdy,2),3)],[-1*Dx(ntopbdy).^(-1).*Dy(ntopbdy).^(-1); -1*Dx(ntopbdy).^(-1).*Dy(ntopbdy).^(-1)],nIJ,cIJ) ...
        + sparse([nbotbdy; nbotbdy],[econnect(nconnect(nbotbdy,1),4); econnect(nconnect(nbotbdy,2),3)],[Dx(nbotbdy).^(-1).*Dy(nbotbdy).^(-1); Dx(nbotbdy).^(-1).*Dy(nbotbdy).^(-1)],nIJ,cIJ);
end
