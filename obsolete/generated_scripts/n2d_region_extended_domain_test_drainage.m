    ...
    casename = 'n2d_region_extended_domain_test_drainage'; % drainage system filename
    initname = 'n2d_region_extended_domain_test_spinup';
    ...
    %% grid and geometry
    load([oo.dn '/' 'BMv5_for_nevis_catchment_filled']); % load Morlighem bedmap (extended for nevis)
    dd = BMv5_filled; dd.skip = 12;
    max_x = 59549.7;
    max_y = 29277.0;
    min_x = -79950.3;
    min_y = -49923.0;
    ...
    min_x = -Inf;
    min_y = -Inf;
    Idx = dd.X_m(:,1)<=max_x & dd.X_m(:,1)>=min_x;
    Idy = dd.Y_m(1,:)<=max_y & dd.Y_m(1,:)>=min_y;
    dd.X_m = dd.X_m(Idx,Idy);
    dd.Y_m = dd.Y_m(Idx,Idy);
    dd.B_m = dd.B_m(Idx,Idy);
    dd.S_m = dd.S_m(Idx,Idy);
    % smooth the bed and surface a bit
    % dd.B_m = imgaussfilt(dd.B_m,5);
    % dd.S_m = imgaussfilt(dd.S_m,5);
    ...
    %% supraglacial lakes
    lakes = nevis_import_lakes();                     % import lake data
    ...
    pp.runoff_function = @(t) 0.75*runoff(((t*ps.t)/pd.td),runoff_2010_nevis140)./ps.m;  % distributed input (m/sec)
