...
casename = 'n2d_0m3s_kappa1e_10_mu1e1_hbreg1e_3_V1e7_drainage'; % drainage system filename
...
% tt = data.tt;                                % load time series from the initial condition
% gg = data.gg;                                % load grid from the initial condition
...
% vv.t = 0;                                       % reset time to zero

%% supraglacial lakes
% pp.x_l = [0.5*L/ps.x];                                          % x-coord of lakes
% pp.y_l = [0.5*W/ps.x];                                          % y-coord of lakes
pp.V_l = [1e7/(ps.Q0*ps.t)];                                      % volume of lakes         
pp.t_drainage = vv.t+ [20*pd.td/ps.t];                            % time of lake drainages (assumed to be the middle time of the Gaussian)
...
% pp.meltE = @(t) (runoff_max/1000/pd.td/ps.m)*(1-exp(-(t+73)/(30*pd.td/ps.t)));
% pp.input_function = @(t) moulin_input*(1-exp(-(t+73)/(30*pd.td/ps.t)))./(ps.m*ps.x^2);     % RACMO moulin input (100 m3/sec)
...
% oo.t_span = [(1:1:pp.t_drainage*ps.t/pd.td-6)*pd.td/ps.t...
%              (pp.t_drainage*ps.t/pd.td-5:0.1:pp.t_drainage*ps.t/pd.td+10)*pd.td/ps.t...
%              (pp.t_drainage*ps.t/pd.td+11:1:365)*pd.td/ps.t];              % time span for simulation (in ps.t)
oo.t_span = vv.t + [(1:0.2:36)*pd.td/ps.t (37:1:365)*pd.td/ps.t];            % time span for simulation (in ps.t)
