...
casename = 'n1d_Vconst_cg0_00_a0_kh0_ks0_mu1e1_c1_V1e7';
...
R_b = ps.x*[tt.Rb];
% R_b = smoothdata(ps.x*[tt.Rb],'movmean',51); % radius of the blister (m)
...
inject_time = 45;
xf = (0.75)^(2/3)*(pd.rho_w*pd.g*0.1/pd.mu)^(1/3)*(V_b/1e5).^(2/3).*((t-t(inject_time))*pd.td).^(1/3);
...
subplot(3,1,1)
plot(t-t(inject_time)-1, R_b, 'k-', 'LineWidth', 2);
hold on
xlabel('Time (days)');
...
plot(t-t(inject_time), xf, 'b--', 'LineWidth', 2);
