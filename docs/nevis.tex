...
\renewcommand{\footrulewidth}{0pt}
\fancyhf{}
\rhead{{\small I J Hewitt, August 2014, modified by H Zhang, Oct 2025}}
\cfoot{\thepage}
}
...
The model consists of a cavity-channel system and a blister layer. Variables and parameters are summarized in Table \ref{tab1}. 

\subsubsection*{Cavity-channel system}
...
\be
\pd{h}{t} + \nabla \cdot \bo{q} + \left[ \pd{S}{t} + \pd{Q}{s} \right] \delta(\bo{x}_c) + \pd{\Sigma}{t} = m + M \delta(\bo{x}_c) + \Sigma_{m} + Q_b,
\ee
...
\ee
and the delta functions apply along the (linear) positions of the conduits $\bo{x}_c(s)$ and the (point) positions of the moulins $\bo{x}_m$. The source term $\Sigma_m$ includes the moulin point sources, defined as
\be
\Sigma_m = \sum_{m}Q_m\delta\left(\boldsymbol{x}_m\right)\mathcal{H}\left(N(\boldsymbol{x}_m)\right),
\ee
where $Q_m$ is the magnitude of the influx, $\boldsymbol{x}_m$ is the moulin position, $\mathcal{H}\left(N(\boldsymbol{x}_m)\right)$ is the Heaviside function in terms of the local effective pressure $N(\boldsymbol{x}_m)$, and $Q_b$ is the leakage from the blister to the cavity-channel system. The definition of $Q_b$ can be found in \ref{sec:blister}.
...
\subsubsection*{\textcolor{blue}{Blister}}\label{sec:blister}
The blister is represented by its thickness $h_b$ and hydraulic potential $\phi_b$. The mass conservation is
\be
...
\ee
where the hydraulic potential is defined as
\be
\phi_b = \rho_w g \left(b+h_b\right) +\rho_i g H + \nabla^2 \left[\frac{E H^3}{12(1-\nu^2)}\nabla^2 h_b\right].
\ee
The leakage term $Q_b$ is formulated as
\be
Q_b = \frac{\kappa h_b}{\mu_{\text{eff}}}|\phi_b-\phi|_{+}.
\ee
...
\textcolor{blue}{$h_b(x,y,t)$}    &\verb|hb|  &     Blister thickness \\
\textcolor{blue}{$\phi_b(x,y,t)$}  &\verb|pb| &   Blister hydraulic potential
\\
\end{tabular}\\
\hline
& \\[-10pt]
...
\textcolor{blue}{$\bo{q}_b(x,y,t)$} 	&\verb|qbx|,\verb|qby| & 	 Discharge in blister \\
\end{tabular} \\
\hline
& \\[-10pt]
\hline
...
\textcolor{blue}{$h_0(x,y)$}   	&\verb|h0| &  	Thickness of the pre-wetted layer\\
\textcolor{blue}{$H(x,y)$}   	&\verb|H| &  	Ice thickness\\
\end{tabular} \\
\hline
& \\[-10pt]
\hline
...
\textcolor{blue}{$\mu_{\text{eff}}$}     &\verb|mu| & Effective viscosity of blister [$10^{1}\un{Pa}\un{s}$] \\
\textcolor{blue}{$E$} & \verb|Ye| &  Young's modulus of ice [$8.8\times10^{9}\un{Pa}$] \\
\textcolor{blue}{$\kappa$} & \verb|kappa_b| &  Blister leakage coefficient [$10^{-10}$] \\
\textcolor{blue}{$h_0$} & \verb|hb_reg1| &  Pre-wetted layer thickness [$0.001\un{m}$] \\
...
By default, the hydraulic potential on indices \verb|gg.nbdy| is set to \verb|phi_a|, and the discharge on all boundary edges \verb|gg.ebdy|, \verb|gg.fbdy| is set to \verb|0|. If other values are to be assigned, these should be included in \verb|aa|; {\it e.g.} \verb|aa.phi| can be defined as a list of values for the hydraulic potential on the nodes \verb|gg.nbdy|. \textcolor{blue}{In addition, the blister hydraulic potential} \verb|pb| \textcolor{blue}{on indices} \verb|gg.nbdy| \textcolor{blue}{is set to} \verb|phi_0-phi_a|.
...
\textcolor{blue}{For the blister, the initial condition is set to} \verb|hb=0|, \verb|pb=phi_0-phi_a|.
...
\subsection*{\textcolor{blue}{Lakes}}
Lakes are defined with a list of node indices \verb|pp.ni_l|. The lake input is assumed to be a Gaussian input. The following code is used to set the positions (\verb|x_l|,\verb|y_l|), volume (\verb|V_l|), timing (\verb|t_drainage|), and timescale (\verb|t_duration|) of drainage events
\begin{verbatim}
>pp.x_l = [0.5*L/ps.x];
>pp.y_l = [0.5*W/ps.x];
>pp.V_l = [0e7/(ps.Q0*ps.t)];        
>pp.t_drainage = [0.5*365*pd.td/ps.t];
...
>[pp.ni_l,pp.sum_l] = nevis_lakes(pp.x_l,pp.y_l,gg,oo);
\end{verbatim}
then finds the node indices where the lake input is imposed.
...
\textcolor{blue}{Additional plotting scripts are provided in} \verb|srcplot|.
