...
\item Simulations of a wintertime lake drainage event in western Greenland show consistent trajectories and speed with remote-sensing observations.
...
The model consists of two coupled water systems: a background cavity-channel drainage system and a transient blister layer (\figref{fig:schematic}a). They occupy separate model domains and have separate hydraulic potentials. The cavity-channel system describes water routed through linked cavities and R-channels, whereas the blister describes water flowing beneath an elastically flexing ice sheet after rapid lake drainage. The two systems exchange water through a leakage term, so that water lost from the blister is gained by the cavity-channel system.

\subsection{Cavity-channel system}\label{sec:method_cavity_channel}
The cavity-channel component follows the two-component subglacial hydrology model of \citeA{hewitt2013Seasonal}. The unknowns are the cavity sheet thickness $h$, the hydraulic potential $\phi$, and the channel cross-sectional area $S$. The cavity sheet and channels share
\begin{linenomath*}
\begin{equation}\label{eq:hydraulic_potential_cavity}
    \phi = \rho_w g b + p_w,
    \qquad
    N = p_i-p_w = \rho_i g H-p_w,
\end{equation}
\end{linenomath*}
where $p_w$ is the water pressure in the cavity-channel system, $N$ is effective pressure, $p_i=\rho_i gH$ is ice overburden, $b$ is bed elevation, and $H$ is ice thickness. The sheet flux is written in the general power-law form used in the numerical model,
\begin{linenomath*}
\begin{equation}\label{eq:sheet_flux}
    \boldsymbol{q}
    =-K_s h^{\alpha_s}\left|\Grad\phi\right|^{\beta_s-1}\Grad\phi,
\end{equation}
\end{linenomath*}
with $\alpha_s=3$ and $\beta_s=1$ in the reference simulations. Channel flux along the channel coordinate $s$ is
\begin{linenomath*}
\begin{equation}\label{eq:channel_flux}
    Q
    =-K_c S^{\alpha_c}\left|\pdiff{\phi}{s}\right|^{\beta_c-1}\pdiff{\phi}{s},
\end{equation}
\end{linenomath*}
with $\alpha_c=5/4$ and $\beta_c=1/2$. These flux laws route water down gradients of hydraulic potential in the sheet and channel networks, respectively.

Mass conservation for the combined cavity sheet and channel storage is
\begin{linenomath*}
\begin{equation}\label{eq:mass_conservation}
    \pdiff{h}{t}
    + \nabla \cdot \boldsymbol{q}
    + \left(\pdiff{S}{t}+\pdiff{Q}{s}\right)\delta(\boldsymbol{x}_c)
    = m + M\delta(\boldsymbol{x}_c) + \Sigma_m^{c} + Q_b,
\end{equation}
\end{linenomath*}
where $\boldsymbol{x}_c$ denotes the channel position, $\delta(\boldsymbol{x}_c)$ localises the channel terms, $m$ is basal melt entering the sheet, and $M$ is channel-wall melt. The cavity opening and closure relation is
\begin{linenomath*}
\begin{equation}\label{eq:cavity_evolution}
    \pdiff{h}{t}
    =
    \frac{\rho_w}{\rho_i}m
    + U_b\frac{h_r-h}{l_r}
    - \frac{2A}{n^n}h|N|^{n-1}N,
\end{equation}
\end{linenomath*}
where the three terms represent melt opening, sliding opening over bed roughness, and viscous creep closure. The basal sliding speed $U_b$ is prescribed in the simulations presented here. The channel area evolves as
\begin{linenomath*}
\begin{equation}\label{eq:channel_evolution}
    \pdiff{S}{t}
    =
    \frac{\rho_w}{\rho_i}M
    - \frac{2A}{n^n}S|N|^{n-1}N,
\end{equation}
\end{linenomath*}
where channel opening is caused by dissipative melting and channel closure is caused by ice creep. We parameterise the channel melting rate as
\begin{linenomath*}
\begin{equation}\label{eq:channel_melting_method}
    M = \frac{\left|Q\,\partial\phi/\partial s\right|
    + \lambda_c\left|\boldsymbol{q}\cdot\Grad\phi\right|}
    {\rho_w L},
\end{equation}
\end{linenomath*}
where $\lambda_c$ is the width over which dissipation in the surrounding sheet contributes to channel melt.

Surface input to the cavity-channel system is represented by point-source moulins,
\begin{linenomath*}
\begin{equation}\label{eq:moulin_cavity}
    \Sigma_m^{c}
    =
    \sum_m Q_m \delta(\boldsymbol{x}_m)\mathcal{H}\left(N(\boldsymbol{x}_m)\right),
\end{equation}
\end{linenomath*}
where $\boldsymbol{x}_m$ is the moulin position and $\mathcal{H}$ is a smoothed Heaviside function in the numerical implementation. This switch sends moulin water to the cavity-channel system when the local effective pressure is non-negative, and diverts the same input to the blister when the cavity-channel system is overwhelmed and $N<0$.

\subsection{Blister model}\label{sec:method_blister}
The blister component describes a water layer of thickness $h_b$ beneath an elastically flexing ice sheet. Its hydraulic potential is
\begin{linenomath*}
\begin{equation}\label{eq:hydraulic_potential_blister}
    \phi_b = \rho_w g b + p_b,
    \qquad
    p_b = \rho_i g H + \rho_w g h_b
    + \nabla^2 \left(B\nabla^2 h_b\right),
\end{equation}
\end{linenomath*}
where $p_b$ is blister water pressure. The three contributions to $p_b$ are ice overburden, the hydrostatic pressure associated with the blister thickness, and elastic bending of the overlying ice. The bending stiffness is $B=E H^3/[12(1-\nu^2)]$, where $E$ is Young's modulus and $\nu$ is Poisson's ratio.

The blister flux follows lubrication theory:
\begin{linenomath*}
\begin{equation}\label{eq:blister_flux}
    \boldsymbol{q}_b
    =
    -\frac{(h_b+h_0)^3}{12\mu_{\text{eff}}}\Grad\phi_b,
\end{equation}
\end{linenomath*}
where $\mu_{\text{eff}}$ is the effective viscosity and $h_0$ is a small pre-wetted-layer regularisation. The blister mass balance is then
\begin{linenomath*}
\begin{equation}\label{eq:mass_conservation_blister}
    \pdiff{h_b}{t}
    + \nabla\cdot\boldsymbol{q}_b
    =
    Q_l + \Sigma_m^{b} - Q_b,
\end{equation}
\end{linenomath*}
where $Q_l$ is lake-drainage input, $\Sigma_m^{b}$ is the part of moulin input diverted into the blister, and $Q_b$ is leakage from the blister into the cavity-channel system. The diverted moulin input is
\begin{linenomath*}
\begin{equation}\label{eq:moulin_blister}
    \Sigma_m^{b}
    =
    \sum_m Q_m \delta(\boldsymbol{x}_m)
    \left[1-\mathcal{H}\left(N(\boldsymbol{x}_m)\right)\right],
\end{equation}
\end{linenomath*}
so that $\Sigma_m^{c}+\Sigma_m^{b}=\sum_m Q_m\delta(\boldsymbol{x}_m)$. For the reference cases, lake drainage is imposed at $\boldsymbol{x}_l$ as a Gaussian pulse,
\begin{linenomath*}
\begin{equation}\label{eq:lake_input}
    Q_l
    =
    \frac{V_l}{\sqrt{2\pi}\tau_l}
    \exp\left[-\frac{(t-t_l)^2}{2\tau_l^2}\right]
    \delta\left(\boldsymbol{x}_l\right),
\end{equation}
\end{linenomath*}
where $V_l$ is lake volume, $\tau_l$ is drainage timescale, and $t_l$ is the drainage time. We use $\tau_l=0.6$ hr so that drainage occurs within hours, consistent with observations of lake hydrofracture events \cite{das2008Fracture,doyle2013Ice,stevens2015Greenland,chudley2019lake}.

The regularisation parameter $h_0$ mimics a pre-wetted layer ahead of the propagating front \cite{hewitt2015Elasticplated}, a common treatment in blister models \citeg{hewitt2018Influence,tobin2023Evolution}. Although $h_0$ influences the propagation velocity, the dependence is weak \cite{peng2020Viscous}. We use $h_0=0.005$ m in the reference cases.

Leakage couples the two model components. In the numerical model it is written as
\begin{linenomath*}
\begin{equation}\label{eq:leakage}
    Q_b
    =
    \frac{\kappa}{\mu_{\text{eff}}}
    h_b
    \left(c_t+\alpha_h h+\alpha_S\overline{S}\right)
    \left[\phi_b-\phi\right]_+,
\end{equation}
\end{linenomath*}
where $\overline{S}$ is the local mean channel cross section and $[\cdot]_+$ denotes the positive part. Thus leakage occurs only where blister potential exceeds the cavity-channel potential. The parameters $\alpha_h$ and $\alpha_S$ allow leakage to depend on local sheet thickness and channel capacity; in the simulations presented here they are set to zero, reducing the coupling to $Q_b=(\kappa/\mu_{\text{eff}})h_b[\phi_b-\phi]_+$. The same $Q_b$ appears with opposite signs in \eqnref{eq:mass_conservation} and \eqnref{eq:mass_conservation_blister}, ensuring mass conservation between the blister and cavity-channel systems.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.85\linewidth]{./figures_new/Fig3_summertime.png}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1.0\linewidth]{./figures_new/Fig5_velocity.png}
