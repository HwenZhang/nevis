...
if ~halfcmap, z_range = [-999 1000]; end
...
aa = nevis_inputs(vv.t,aa,pp,gg,oo); % calculate inputs
...
%% set up figure
figure(1); clf; set(gcf,'PaperPositionMode','auto','Units','centimeters','Position',[5 2 30 15]);
set(0,'DefaultAxesFontSize',10);
set(0,'DefaultTextFontSize',10);
axes_size = [0.075 0.1 0.4 0.8;
            0.55 0.1 0.4 0.8];
...
%     hand = pcolor(xx,yy,zz); set(hand,'linestyle','none'); shading interp;
    ...
    ccx(ai) = colorbar('peer',gca,'horizontal','position',[tmp(1) tmp(2)+tmp(4) 0 0]+cbar_size,'XAxislocation','top');
%     text(1.1,0,'m','units','normalized','Parent',cx(ai),'VerticalAlignment','bottom','HorizontalAlignment','left');
    if ver==1,
    text(-0.05,0,'z [ m ]','units','normalized','Parent',ccx(ai),'VerticalAlignment','bottom','HorizontalAlignment','right');
    elseif ver==2,
    ccx(ai).Label.String = 'z [ m ]'; ccx(ai).Label.Units = 'normalized'; ccx(ai).Label.Position = [-0.05 0]; ccx(ai).Label.VerticalAlignment = 'bottom'; ccx(ai).Label.HorizontalAlignment = 'right';
    end
...
ax(ai) = axes('position',axes_size(ai,:));
    ...
    tmp = get(gca,'position'); cbar_size = [tmp(3)*3/16 0.01  tmp(3)*5/8 0.02];
    ccx(ai) = colorbar('peer',gca,'horizontal','position',[tmp(1) tmp(2)+tmp(4) 0 0]+cbar_size,'XAxislocation','top');
    tmp = get(ccx(ai),'XTick'); labels = 10.^tmp; set(ccx(ai),'XTickLabel',labels);
%     text(1.1,0,'m^2 s^{-1}','units','normalized','Parent',ccx(ai),'VerticalAlignment','bottom','HorizontalAlignment','left');
    if ver==1,
    text(-0.05,0,'q [ m^2/s ]','units','normalized','Parent',ccx(ai),'VerticalAlignment','bottom','HorizontalAlignment','right');
    elseif ver==2,
    ccx(ai).Label.String = 'q [ m^2/s ]'; ccx(ai).Label.Units = 'normalized'; ccx(ai).Label.Position = [-0.05 0]; ccx(ai).Label.VerticalAlignment = 'bottom'; ccx(ai).Label.HorizontalAlignment = 'right';
    end
...
set(ccx(1),'XLim',CLim(1,:)); set(ccx(2),'XLim',CLim(2,:));
set(ccx(1),'XLim',z_range);
