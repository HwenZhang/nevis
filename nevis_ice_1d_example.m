...
pd.n_glen = 1;
pd.A_glen = 1/2/((0.001)*pd.rho_i*pd.g*ps.z*ps.x/pd.u_b); % to make dimensionless viscosity term a given size;
[pd,ps,pp,oo] = nevis_add_parameters_ice(pd,ps,pp,oo); % add parameters etc needed to solve for ice velocity

gg = nevis_label_ice(gg); % add boundary labels needed for ice velocity (different to those needed for hydrology)
