...
oo.casename = 'n2d_RACMO_test_regional_eps1e_02_kappa1e_10_mu2e1_partition1e_01_spinup';  % casename
casename = 'test';
...
pp.t_drainage = 0.5*pd.td/ps.t*(lakes.drainage_time_start + lakes.drainage_time_end);  
                                                  % time of lake drainages (assumed to be the middle time of the Gaussian)
...
pp.runoff_function = @(t) runoff(((t*ps.t)/pd.td),runoff_2022_nevis140_smooth)./ps.m;  % distributed input (m/sec)
...
% Set the time span for the model run (with higher resolution around the time of lake drainage)
